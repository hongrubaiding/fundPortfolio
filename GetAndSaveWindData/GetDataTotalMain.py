...
class GetDataTotalMain:
    ...
    def get_index_constituent(self, indexCode='000300.SH', getDate='2020-02-01'):
        '''
        获取指数成分股
    ...
    def get_lackdata_to_MySql(self, code, startDate, endDate, code_style='index'):
        '''
        ...
        if (datetime.strptime(end_date, "%Y-%m-%d") - timedelta(days=0)).strftime("%Y-%m-%d") <= start_date:
    ...
    def get_date_from_MySql(self, code, start_date, end_date, code_style='index', name_list=['close_price']):
        if not name_list:
    ...
    def get_hq_data(self, code, start_date, end_date, code_style='index', name_list=['close_price']):
        '''
        获取行情数据主入口
        :param code:
        :param start_date:
        ...
        resultDf = self.get_date_from_MySql(code, start_date, end_date, code_style, name_list)
        return resultDf
    ...
    def get_fund_size(self, code_list=[]):
        ...
        wsdData = w.wsd(codes=code_list,fields="amt", beginTime=start_date,endTime=end_date)
        if wsdData.ErrorCode != 0:
            self.logger.error("wind获取场内基金日均成交额有误，错误代码" + str(wsdData.ErrorCode))
            return pd.DataFrame()
        result_Se = pd.DataFrame(wsdData.Data, index=wsdData.Codes, columns=wsdData.Times).T.median()
    ...
    def get_fund_base_info(self, fund_code_list=[]):
        wssdata = w.wss(codes=fund_code_list,
                        fields=["fund_firstinvesttype", "fund_investtype", "fund_setupdate", "fund_trackindexcode",
                                "fund_fullname"])
        if wssdata.ErrorCode != 0:
            self.logger.error("wind获取场内基金日均成交额有误，错误代码" + str(wssdata.ErrorCode))
            return pd.DataFrame()
        resultDf = pd.DataFrame(wssdata.Data, index=wssdata.Fields, columns=wssdata.Codes).T
        name_dic = {"fund_firstinvesttype".upper(): "基金类型", "fund_investtype".upper(): "产品类型",
                    "fund_setupdate".upper(): "基金成立日", 'fund_trackindexcode'.upper(): "跟踪指数代码", "fund_fullname".upper(): "基金全称"}
        resultDf.rename(columns=name_dic,inplace=True)
        return resultDf


if __name__ == '__main__':
    GetDataTotalMainDemo = GetDataTotalMain('ifind')
